...
CREATE POLICY "Agency users can update assigned shifts."
  ON shifts FOR UPDATE
  USING (
  ...
  USING ( worker_id = auth.uid() OR
    (
      EXISTS (
        SELECT 1 FROM profiles
        WHERE profiles.id = auth.uid() 
        AND (profiles.role = 'admin' OR profiles.role = 'agency' OR profiles.role = 'business')
      )
    )
  );
...
CREATE POLICY "Business and agency users can update application status."
  ON applications FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM shifts
      WHERE shifts.id = applications.shift_id
      AND (shifts.business_id = auth.uid() OR shifts.agency_id = auth.uid())
    ) OR
    (
      EXISTS (
        SELECT 1 FROM profiles
...
-- RLS policies for market_updates
CREATE POLICY "Market updates are viewable by everyone."
  ON market_updates FOR SELECT
  USING ( true );

CREATE POLICY "Admin users can insert market updates."
  ON market_updates FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid() 
      AND (profiles.role = 'admin' OR profiles.role = 'agency')
    )
  );

CREATE POLICY "Admin users can update market updates."
  ON market_updates FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admin and agency users can create organizations."
...
-- Create stored procedure for assigning shifts to workers
CREATE OR REPLACE FUNCTION assign_shift_to_worker(
  shift_id UUID,
  worker_id UUID
)
RETURNS VOID AS $
BEGIN
  -- Update shift status and worker_id
  UPDATE shifts
  SET worker_id = assign_shift_to_worker.worker_id,
      status = 'assigned',
      updated_at = now()
  WHERE id = shift_id
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Shift not available for assignment';
  END IF;
  ...
  SET status = 'accepted',
      updated_at = now()
  WHERE shift_id = assign_shift_to_worker.shift_id
  AND worker_id = assign_shift_to_worker.worker_id;
  
  -- Notify other applicants that the shift is filled
  UPDATE applications
  SET status = 'rejected',
      admin_notes = 'Shift was assigned to another worker',
      updated_at = now()
  WHERE shift_id = assign_shift_to_worker.shift_id
  AND worker_id <> assign_shift_to_worker.worker_id
  AND status = 'pending';
